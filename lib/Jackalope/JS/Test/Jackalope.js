                ...
                for (var i = 0; i < tests.length; i++) {
                    ...
                    for (var j = 0; j < test["pass"].length; j++) {
                        tester.validation_pass(
                            ...
                            "... validation passed for " + JSON.stringify(test["pass"][j]) + " against  " + JSON.stringify(schema)
                        )
                    }
                    ...
                    for (var k = 0; k < test["fail"].length; k++) {
                        tester.validation_fail(
                            ...
                            "... validation failed correctly for " + JSON.stringify(test["fail"][k]) + " against  " + JSON.stringify(schema)
                        )
                    }
                }
            }
