...
Jackalope.Client.Resource.prototype.init = function ( opts ) {
    // NOTE:
    // I think it best I not require these
    // so that I can better use this object
    // for creation as well as editing.
    // - SL
...
Jackalope.Client.Resource.prototype.set = function ( attrs, options ) {
    ...
    for (var k in attrs) {
        ...
        this._traverse_path_and_set( k, this._body, attrs[ k ], k );
        if (!options.silent) {
...
Jackalope.Client.Resource.prototype._check_schema = function ( path, value ) {
    ...
    if ( path.indexOf('.') != -1 ) {
        ...
        var copy = this._body[ path ] == undefined ? {} : Jackalope.Util.clone( this._body[ path ] );
