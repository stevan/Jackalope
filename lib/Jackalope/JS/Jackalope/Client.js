...
Jackalope.Client.Resource.prototype.set = function ( attrs, options ) {
    ...
    for (var k in attrs) {
        if ( this.has_associated_schema() ) {
            if ( this.schema.properties[ k ] == undefined && this.schema.additional_properties[ k ] == undefined ) {
                throw new Jackalope.Client.Error ("The property (" + k + ") is not a valid property for this resource");
            }
            var prop_schema = this.schema.properties[ k ] != undefined
                ? this.schema.properties[ k ]
                : this.schema.additional_properties[ k ];
            var result = this.schema_repository.validate( prop_schema, attrs[ k ] );
            if (result.error) {
                throw new Jackalope.Client.Error ("The property (" + k + ") failed to validate", result.error);
            }
        }
