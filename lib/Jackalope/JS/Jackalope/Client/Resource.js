...
Jackalope.Client.Resource.prototype._check_schema = function ( path, value ) {
    ...
    if ( path.indexOf('.') != -1 ) {
        ...
        this.traverse_path_and_set( parts.join('.'), copy, value, orig_path );
        value = copy;
    }
    // now find the schema ...
    var prop_schema;
...
Jackalope.Client.Resource.Repository.prototype = {
    ...
    "create" : function ( data, success, error ) {
        ...
        var result = this.schema_repository.validate( { '$ref' : this.schema.id }, data );
        if (result.error) {
            throw new Jackalope.Client.Error ("Resource failed to validate", result.error);
        }
    ...
    "edit" : function ( resource, success, error ) {
        ...
        var result = this.schema_repository.validate( { '$ref' : this.schema.id }, resource.body );
        if (result.error) {
            throw new Jackalope.Client.Error ("Resource failed to validate", result.error);
        }
